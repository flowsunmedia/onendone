...
export default async (req) => {
  ...
  if (act === "update") {  // player edits self (or admin edits anyone)
  ...
  if (act === "createGroup") {
    ...
    if (!g) { g = { id: id(), startHole: sh, createdAt: Date.now() }; d.groups.push(g); }
  ...
  if (act === "join") {
    const g = d.groups.find(x => x.id === body.groupId); const p = d.players.find(x => x.id === body.playerId);
    if (!g || !p) return json({ error: "no" }, 404);
    const size = d.players.filter(x => x.groupId === g.id).length;
    if (size >= 6 && !adminOk) return json({ error: "full" }, 409);
    p.groupId = g.id; await write(store, ev, d); return json({ ok: true, data: d });
  }
  if (act === "leave") {
    const p = d.players.find(x => x.id === body.playerId); if (p) p.groupId = null;
